...
- `R14.3` — [v2.3/v2.15] Tables por USOS: skill `tables` (modelo USES R7.4; label UI "Tables") — **cero base gratis**; cada uso/partida se compra con `buyTablesUp` (que además sube el techo permTiles y hace `uses += 1` mid-run); `openRun` repone `uses = usesBought`. Tocar baldosa apagada consume 1 uso y la activa ESA partida. SIN costo de monedas por activación (`runTilePrice ≡ 0`; RUN_TILE_BASE OBSOLETO-v2.2).
