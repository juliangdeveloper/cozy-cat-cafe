...
- `R1.5` — **[v2.18] First-run tutorial:** `settings.seenTutorial` defaults to `false` in `createGame`. UI shows a 5-step spotlight overlay after the café opens (run scene): stack food; matching piles auto-serve; win = serve everyone; Close café if stuck (keep coins); Shop = skills. Board + trays stay playable on every step; non-spotlight UI is blocked. Close café is visible on its step but not tappable until the tutorial ends. **[v2.19]** Manual close is press-and-hold 3s (`CLOSE_HOLD_MS`); ↻ 90° remaps hex centers (90/270 = flat-top lattice; tiles stay upright). Not a CSS rotate of the board. Skip or completing the last step → `seenTutorial = true` + persist (never auto-show again on that save). **Old saves** (v2.16/v2.17) without the flag: `deserializeState` sets `seenTutorial = true` so existing players skip it. New-game reset (`createGame`) shows the tutorial again.
...
```
- `R7.1` — **[v2.16 OBSOLETO]** Café Level ya NO desbloquea skills. `cafeLevel = totalGames+1` se sigue actualizando por compat de saves, pero `buySkill` no lo consulta. Todas las skills se compran con monedas desde el inicio. → US-20/21 retirados del gate.
...
- `R7.3` — **Compra (v2.3/v2.15/v2.16 = todo usos):** `buySkill(power)` exige `coins ≥ precio siguiente = price × 1.35^usesBought` (sin gate cafeLevel). La 1ª compra desbloquea y **ES el 1er uso** (`usesBought += 1`, `uses += 1`, `owned=true`); recomprar = +1 uso ya (acumulativo; **no** `uses = usesBought` — eso devolvería gastados mid-run). → US-17/18/19, US-23.
...
- `R7.6` — **SWAP PILES** (medio, `price=120`): `useSwapPiles(state, cellA, cellB)` — **intercambia los `stack` completos** de 2 celdas (o 1 pila de pool ↔ 1 celda). Marca `uses -= 1`. No toca bloqueadas. **v2.6 FIX:** tras el swap el APP dispara la cascada (R12.2: mutación de topes → re-evaluar) — antes solo re-renderizaba y los merges pendientes quedaban congelados hasta la próxima colocación. → US-18.
...
- `R12.2` — **CASCADA (v3 / v2.17):** tras toda mutación de topes (colocar, auto-servir, swap) re-evaluar merges+serves hasta estabilizar; UN grupo por eslabón (paso visible al jugador); 1 eslabón = `CASCADE_STEP_MS=600ms` (CONFIG). Orden por eslabón: merge (grupo completo, multi-fuente) → auto-servir (ids ascendentes) → siguiente eslabón. **Las pilas ≥10 NO se destruyen durante la cadena** (siguen participando en merges).
- `R12.3` — **UMBRAL [v2.17]:** grupo contiguo ≥ `DEBRIS_THRESHOLD=10` fichas (CONFIG) se destruye **solo después de que la cadena de merges/serves de esa acción del jugador se estabiliza** (`sweepDebrisRuns` al final de `resolveCascade` / espejo `cascadeLinks`). Bonus `DEBRIS_BONUS_PER=25` × qty (CONFIG ⚖BALANCE). No reentra merge tras el barrido.
- `R12.4a` — destroyPile queda **PENDIENTE DE BALANCE** (pierde valor relativo frente a R12.3).
- `R12.4b` — **Ancla del jugador [v2.11]**: colocar una pila **MONOCOLOR** (por pool o firma explícita) marca `run.anchor = cellId`. Durante toda la cascada que esa colocación dispara, TODO grupo contiguo que **contenga** el ancla tiene como destino el ancla — los vecinos y vecinos-de-vecinos (revelados en eslabones siguientes) drenan su run de tope HACIA la baldosa del jugador. Grupos que NO contienen el ancla usan el árbitro normal (T1/R2). El ancla se **elimina** al final de `resolveCascade` (no ensucia saves ni estados sin ancla); multicolor NO ancla; swap/unlock no marcan ancla. Escombros intactos: si el ancla acumula ≥10, explota (R12.3). El espejo `cascadeLinks` (index.html) replica la misma regla (espejo≡oráculo, v2.2). Tests: test/v2.t21.test.js (T21a..T21f).
- `R12.4c` — **Imán monocolor (inversa) [v2.12]**: colocar una pila **MULTICOLOR** marca `run.monoSink = true` (junto a `anchor = null`). Durante toda la cascada que esa colocación dispara, TODO grupo del eslabón que contenga una celda **PURAMENTE monocolor** (stack ENTERO = un solo color) la elige como destino — el run de tope del jugador drena HACIA el vecino puro. Empate entre puras: la MÁS ALTA (tie: menor índice). Sin pura en el grupo: árbitro T1/R2 normal. Prioridad del árbitro: ancla > imán > T1/R2 (una colocación es mono O multi: nunca coexisten). Simetría R12.4b: mono→tu baldosa recibe; multi→el vecino puro recibe. `monoSink` se elimina al final de la cascada; swap/unlock NO activan el imán. El espejo `cascadeLinks` replica la regla. Tests: test/v2.t22.test.js (T22a..T22d).
...
- `R13.7` — Colores: 10 máx, 4 de inicio. Compra DIRECTA en tienda: `buyColor` desbloquea el siguiente color del roster; precio `COLOR_PRICE(n) = COLOR_PRICE_BASE * (n-3)` CONFIG ⚖BALANCE. R10 queda REPLANTEADA: `productsBought`/catálogo se ELIMINA del state (§1); `colorsUnlocked`/`colorsOwned` pasa a derivarse de compras directas. R10.2 (solo colores desbloqueados se generan) se mantiene como principio. R10.1/R10.3 y R6.3 quedan marcadas OBSOLETAS-v2 (reemplazadas por R13.7). **v2.6.1 FIX (render):** los IDs 7..10 existían en la lógica pero la capa app solo mapeaba 1..6 (`TILE_CSS`/`colorName`/tokens CSS) → caían al fallback `--tile-mint` y fichas de IDs DISTINTOS se dibujaban idénticas (no fusionaban: 1≠7, correcto, pero se veía como bug). Paleta 7..10 = denim `#2c6eb0`, teal `#1f8f81`, pine `#14532d`, orchid `#9932cc` (elegida por ΔE94 vs los 6 previos, ≥25.9 en el peor par; ver audit_v261_dom.mjs).

### R14. Tablero dual rectangular 6×6 pointy 36 [v2.14] (modifica R6.2, R8)
- `R14.1` — Tablero SIEMPRE dibujado completo: RECTÁNGULO pointy de **6 filas axiales × 6 celdas = 36 [v2.14]** (v2.13: 5×7=35; v2.12 y antes: 4×8=32). Contorno rectangular tipo marco de Catan con offset de panal; en columna plegada `q+floor(r/2)` las 6 filas comparten patrón consecutivo -3..2. No jugable = visible apagada (estilo lock). (v2.0/v2.1: panal [7,9,9,7], reemplazado.)
...
- `R14.3` — [v2.3/v2.15/v2.17] Tables por USOS: skill `tables` (modelo USES R7.4; label UI "Tables" / icon `power_activate` / mode `activate`) — **cero base gratis**; cada uso/partida se compra con `buyTablesUp` (que además sube el techo permTiles y hace `uses += 1` mid-run); `openRun` repone `uses = usesBought`. **Tap corto** en el botón Tables entra/sale del modo activar; tocar baldosa apagada elegible consume 1 uso y la activa ESA partida. **Press-and-hold** (~`TABLES_HOLD_MS=550`) muestra filler radial y llama `activateAroundUnlocked` (batch). SIN costo de monedas por activación (`runTilePrice ≡ 0`; RUN_TILE_BASE OBSOLETO-v2.2).
- `R14.3b` — **[v2.17] Hold-Tables batch (`activateAroundUnlocked`):** snapshot al inicio de dormant que (1) son vecinas hex del set desbloqueado actual y (2) cumplen R14.6 (≥2 vecinos desbloqueados). Orden estable = **índice de board ascendente**. Activa en serie vía `activateTile` hasta agotar `uses` o candidatos; si uses se agotan mid-batch, para. Errores: `locked` / `noUses` / `noneEligible`. El set desbloqueado **no crece mid-batch** (una sola corona alrededor del snapshot).
- `R14.4` — [v2.5 dial / v2.15] Compra permanente en TIENDA (`buyTablesUp`, fila "Tables", subtítulo `N → +1 per run`): `permTilePrice = TABLES_PERM_BASE(80) × 1.25^permTiles` (BALANCE_REPORT.md §6) → `permTiles += 1` Y `skills.tables.usesBought += 1` Y `uses += 1` (usable ya mid-run; sin devolver gastados). La 1ª compra marca `tables.owned`. NO activa ninguna celda. **v2.4:** techo = `celdas del tablero − 7` (**29 con 36 [v2.14]**); tope ⇒ `{error:'maxUses'}`. Extra table (`progress.clients`) OBSOLETA-v2.15 — fuera de tienda; Capacity es la expansión de clientes.
...
- `R17.2` — **Mejora de usos (v2.15):** cada skill modelo uses puede subir +1 uso por partida: `buyUsesUp`, precio `USES_UP_BASE × USES_UP_RATIO^usesBought` (CONFIG 60, 1.6 ⚖BALANCE). Mid-run: `usesBought += 1` y `uses += 1`. openRun: `uses = usesBought`.
...
- `R18.4` — **Recarga al agotarse (v2.10.1 anti-colapso)**: cuando el inventario de un color llega a 0 (`bag[color] === 0`), el rerolleo cae uniforme entre los colores **MUERTOS** (incluido el que acaba de morir — se mantiene "puede ser el mismo o no") y entra con puñado aleatorio en `[CONFIG.BAG_RELOAD_MIN, CONFIG.BAG_RELOAD_MAX]` (6..14). **NUNCA sobre un color vivo**: en v2.10.0 la recarga podía caer en un vivo y el nº de vivos era monótono decreciente → colapsó a 1 vivo (save real `{"10":4}`, pool 100% color 10). Invariante: vivos === min(4, cu) tras cualquier draw; `drawPoolPiles` SANA bolsas heredadas sub-viudas (revive muertos con puñados frescos) antes de dibujar — el save colapsado se repara solo en el próximo refill. Transición entre colores gradual, nunca de golpe.
...
### T6. Poderes / árbol
- `T6.1` — **[v2.16] sin gate cafeLevel:** GIVEN cafeLevel 1 y monedas; THEN `swapPiles`/`destroyPile`/`refreshPool` comprables. [R7.1 OBSOLETO]
...
- `T11.3` — **orden determinista de eslabón [v2.17]:** GIVEN una colocación que dispara merge + pedidos servibles + grupo ≥ umbral; THEN por eslabón: merge → auto-servir; la destrucción umbral ocurre **solo tras estabilizar** toda la cadena (`sweepDebrisRuns`). [R12.2, R12.3]
...
- `T13.7` — **[v2.17] 2-neighbor unlock:** GIVEN núcleo desbloqueado; WHEN activar celda con 1 vecino desbloqueado; THEN `{error:'needTwoNeighbors'}`. WHEN activar celda con ≥2; THEN ok. [R14.6]
- `T13.8` — **[v2.17] hold-batch:** GIVEN uses=N y K elegibles (≥2 vecinos + around unlocked); WHEN `activateAroundUnlocked`; THEN activa min(N,K) en orden de índice board; stops on uses=0; no activa no-vecinas. [R14.3b]
- `T13.9` — **[v2.17] debris after chain:** GIVEN merges que acumulan ≥10 mid-cadena; THEN la pila 10+ sobrevive eslabones intermedios y solo se destruye al final. [R12.3]
