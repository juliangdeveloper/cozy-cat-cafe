...
- `R7.3` — **Compra (v2.3/v2.15 = todo usos):** `buySkill(power)` exige `cafeLevel ≥ unlockLevel` Y `coins ≥ precio siguiente = price × 1.35^usesBought`. La 1ª compra desbloquea y **ES el 1er uso** (`usesBought += 1`, `uses += 1`, `owned=true`); recomprar = +1 uso ya (acumulativo; **no** `uses = usesBought` — eso devolvería gastados mid-run). → US-17/18/19, US-23.
