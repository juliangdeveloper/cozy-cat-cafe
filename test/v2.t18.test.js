...
// ---------------------------------------------------------------------------
test('T18c [R12.1 v2.2] merge: el vecino cede su racha completa y queda VACÍO (A=[])', () => {
  need('placeStack');
  const s = mkGame(1);
  ...
  assert.deepEqual(st.run.board[0].stack, [], 'RED: A debe quedar VACÍO tras ceder su racha (R12.1 v2.2, sin ficha de reserva)');
});
...
test('T18d [R3.5 v2.2] colocar pila en baldosa vacía sigue OK (regresión)', () => {
  ...
  const st = unwind(ret, s);
  assert.equal(st.run.board[idx].stack.length, 1, 'RED: la pila del pool debe quedar en la celda');
